...
if(isset($_POST['submit']))
{
	...
	else
	{
		...
		if($qsql = mysqli_query($con,$sql))
		{	
			$presamt = (float)$_POST['cost'] * (float)$_POST['unit'];
...
?>
    <form method="post" action="" name="frmpresrecord" onSubmit="return validateform()">
        <input type="hidden" name="prescriptionid" value="<?php echo isset($_GET['prescriptionid']) ? mysqli_real_escape_string($con, $_GET['prescriptionid']) : ''; ?>" />
        <table class="table table-striped">
            <tbody>
                ...
                <tr>
                    <td>Cost</td>
                    <td><input class="form-control" type="text" name="cost" id="cost" value="" readonly style="background-color:pink;" /></td>
                </tr>
                <tr>
                    <td>Unit</td>
                    <td><input class="form-control" type="number" min="1" name="unit" id="unit" value="" onkeyup="calctotalcost(cost.value,this.value)" onchange="" /></td>
                </tr>
                <tr>
                    <td>Total Cost</td>
                    <td><input class="form-control" type="text" name="totcost" id="totcost" value="" readonly style="background-color:pink;" /></td>
                </tr>
                <tr>
                    <td>Dosage</td>
                    <td><select class="form-control show-tick" name="select2" id="select2">
                            ...
                            foreach ($arr as $val) {
                                $selected = ($val == $rsedit['dosage']) ? 'selected' : '';
...
?>
	...
	<table>
	<tr><td>
	 <center><a href='patientreport.php?patientid=<?php echo $_GET[patientid]; ?>&appointmentid=<?php echo $_GET[appid]; ?>'><strong>View Patient Report>></strong></a>
</center>
	</td></tr>
	</table>
<script>
